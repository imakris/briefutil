...
public:
                    ...
                    const QByteArray&  json_payload)
    {
        ...
        if (dir.exists() && !dir.removeRecursively()) {
            error = "cannot clear temporary profile directory " + qs(tmp_dir);
            return;
        }
        if (!QDir().mkpath(tmp_dir)) {
            error = "cannot create temporary profile directory " + qs(tmp_dir);
            return;
        }
        ...
        if (out.write(json_payload) != json_payload.size()) {
            error = "incomplete profile write to " + qs(profile_path);
            return;
        }
        out.close();
