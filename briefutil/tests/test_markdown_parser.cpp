...
template<typename T>
static const T* get_block(const body_block_t& b)
{
...
int main()
{
    // -- Plain text (no markdown) --
    {
        ...
        auto* p = get_block<paragraph_block_t>(blocks[0]);
        ASSERT(p, "plain text should be a paragraph_block_t");
    ...
    // -- Two paragraphs separated by blank line --
    {
        ...
        ASSERT(get_block<paragraph_block_t>(blocks[0]), "first is paragraph");
        ASSERT(get_block<paragraph_block_t>(blocks[1]), "second is paragraph");
    ...
    // -- ATX headings --
    {
        ...
        auto* h1 = get_block<heading_block_t>(blocks[0]);
        auto* h2 = get_block<heading_block_t>(blocks[1]);
        auto* h3 = get_block<heading_block_t>(blocks[2]);
    ...
    // -- Heading with inline formatting --
    {
        ...
        auto* h = get_block<heading_block_t>(blocks[0]);
    ...
    // -- Image --
    {
        ...
        auto* img = get_block<image_content_block_t>(blocks[0]);
    ...
    // -- Table --
    {
        ...
        auto* tb = get_block<table_block_t>(blocks[0]);
    ...
    // -- Mixed content --
    {
        ...
        ASSERT(get_block<heading_block_t>(blocks[0]), "first is heading");
        ASSERT(get_block<paragraph_block_t>(blocks[1]), "second is paragraph");
        ASSERT(get_block<list_block_t>(blocks[2]), "third is list");
        ASSERT(get_block<image_content_block_t>(blocks[3]), "fourth is image");
        ASSERT(get_block<table_block_t>(blocks[4]), "fifth is table");
